...
export const TelephonyProviderNameSchema = z.enum(["twilio"]);
export type TelephonyProviderName = z.infer<typeof TelephonyProviderNameSchema>;
...
export const CallRecordSchema = z.object({
  callId: z.string(),
  providerCallId: z.string().optional(),
  provider: TelephonyProviderNameSchema,
...
export type InitiateCallInput = {
  ...
  /** Inline TwiML to execute (optional for debugging) */
  inlineTwiml?: string;
};
...
export type OutboundCallOptions = {
  /**
   * Text to speak immediately (for "notify" mode).
   * If used in "conversation" mode, this acts as the "system prompt" or goal.
   */
  message?: string;
  /** Call mode (overrides config default) */
  mode?: CallMode;
  /** Tool profile override for this call (e.g. "minimal", "messaging", "full") */
  profile?: string;
};
